...
////////////////////////////////////////////////////////
	...
	function comparar_correo($correo, $telf){ //comparar correo para no registrar 2 iguales
		
		$conexion=conexion();
		$contador=0;
		$sql="SELECT CORREO, TELEFONO FROM dato_persona WHERE CORREO = :correo OR TELEFONO = :telef";
		$resultado=$conexion->prepare($sql);
		$resultado->execute(array(":correo"=>$correo, ":telef"=>$telf));
		while($consulta=$resultado->fetch(PDO::FETCH_ASSOC)){
			
			if($consulta['CORREO']==$correo || $consulta['TELEFONO']==$telf)	{				
				$contador++;
			}				
		}
		if($contador>0){
